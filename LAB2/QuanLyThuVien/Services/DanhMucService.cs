...
namespace QuanLyThuVien.Services
{
    public class DanhMucService
    {
        ...
        {
            return Db.Query(
                @"SELECT MaNhanVien, Ho, Ten, Phai, NgaySinh, ChucVu, SoDienThoai
                  FROM NhanVien
                  ORDER BY MaNhanVien");
        }
            ...
            bool capNhat)
        {
            ...
            try
            {
                ...
                int n = Db.Execute(
                    ...
                    new SqlParameter(
                        "@SDT",
                        (object)(sdt ?? string.Empty))
                );

                return n > 0
                    ? KetQuaXuLy.Ok(
                        "Lưu nhà xuất bản thành công.")
                    : KetQuaXuLy.Loi(
                        "Không có dữ liệu được thay đổi.");
            }
            catch (SqlException ex)
            ...
            string ma)
        {
            ...
            if (index < 0 ||
                cotChoPhep[index] != cotMa)
            {
                return KetQuaXuLy.Loi(
                    "Tham số xóa không hợp lệ.");
            }
            ...
            catch (SqlException ex)
            {
                ...
                {
                    return KetQuaXuLy.Loi(
                        "Không thể xóa vì dữ liệu đang được sử dụng ở nghiệp vụ khác.");
                }
